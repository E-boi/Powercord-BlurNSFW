...
let blockedChannels = Array(), blurChannels = Array()
...
module.exports = class Blur extends React.PureComponent {
  ...
  render() {
    ...
    return <>
      ...
      <Category name='Block Channels' description='Block channels from getting effected.' opened={this.state.category0Opened} onChange={() => this.setState({ category0Opened: !this.state.category0Opened })}>
        {blockedChannels.map((channel, idx) => (
          <div>
            <TextInputWithButton
              ...
              onChange={val => { updateSetting('Blocked', this.handleBlockChannelChange(idx, val)) }}
              buttonOnClick={() => { updateSetting('Blocked', this.handleRemoveBlockChannel(idx)) }}
              buttonText='Remove'
        ...
        <Button
          onClick={() => { updateSetting('Blocked', this.handleBlockChannel()) }}
        >
          Add channel
      </Button>
      </Category>
      <Category name='Blur Channels' description='Blur channels that aren`t affected.' opened={this.state.category1Opened} onChange={() => this.setState({ category1Opened: !this.state.category1Opened })}>
        {blurChannels.map((channel, idx) => (
          <div>
            <TextInputWithButton
              ...
              placeholder={'Enter a channel/user id'}
              defaultValue={channel.id}
              onChange={val => { updateSetting('Blur', this.handleBlurChannelChange(idx, val)) }}
              buttonOnClick={() => { updateSetting('Blur', this.handleRemoveBlurChannel(idx)) }}
              buttonText='Remove'
              buttonIcon='fal fa-minus'
            />
          </div>
        ...
        <Button
          onClick={() => { updateSetting('Blur', this.handleBlurAddChannel()) }}
        >
          Add channel
      </Button>
      </Category>
